...
ti.init(arch=ti.opengl)  # ti.cpu ti.gpu ti.vulkan ti.opengl ti.metal(macOS)
resolution = width, height = (1300, 650)
...
class PyShader:
    def __init__(self, app):
        self.app = app
        self.screen_array = np.full((width, height, 3), [0, 0, 0], np.float32)
        # taichi fields
        self.screen_field = ti.Vector.field(3, ti.float32, (width, height))
    ...
    def render(self, time: ti.float32):
        ...
        for frag_coord in ti.grouped(self.screen_field):
            ...
            self.screen_field[int(frag_coord.x), int(resolution.y - frag_coord.y)] = col * 255
